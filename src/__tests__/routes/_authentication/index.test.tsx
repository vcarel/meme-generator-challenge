  ...
  describe("MemeFeedPage", () => {
    ...
    it("should fetch the memes and display them with their comments", async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId("meme-description-dummy_meme_id_1")).toHaveTextContent(
          "dummy meme 1",
        );

        // We check that the right number of comments is displayed
        expect(screen.getByTestId("meme-comments-count-dummy_meme_id_1")).toHaveTextContent(
          "3 comments",
        );

        // We check that the right comments with the right authors are displayed
        expect(
          screen.getByTestId("meme-comment-content-dummy_meme_id_1-dummy_comment_id_1"),
        ).toHaveTextContent("dummy comment 1");
        expect(
          screen.getByTestId("meme-comment-author-dummy_meme_id_1-dummy_comment_id_1"),
        ).toHaveTextContent("dummy_user_1");

        expect(
          screen.getByTestId("meme-comment-content-dummy_meme_id_1-dummy_comment_id_2"),
        ).toHaveTextContent("dummy comment 2");
        expect(
          screen.getByTestId("meme-comment-author-dummy_meme_id_1-dummy_comment_id_2"),
        ).toHaveTextContent("dummy_user_2");

        expect(
          screen.getByTestId("meme-comment-content-dummy_meme_id_1-dummy_comment_id_3"),
